...
function wavelengthToPalette(wl: number): Palette {
  if (wl < 100) {
    return { primary: [230, 80, 65], secondary: [260, 70, 55], glow: [215, 90, 80] }
  } else if (wl < 200) {
    return { primary: [200, 75, 60], secondary: [220, 65, 50], glow: [190, 85, 75] }
  } else if (wl < 500) {
    return { primary: [40, 85, 60],  secondary: [25, 80, 50],  glow: [50, 90, 75]  }
  } else if (wl < 1500) {
    return { primary: [10, 80, 45],  secondary: [350, 75, 40], glow: [20, 85, 65]  }
  } else {
    return { primary: [320, 70, 40], secondary: [300, 65, 35], glow: [330, 80, 60] }
  }
}
...
function instrumentToStyle(instrument: string): InstrumentStyle {
  ...
  return {
    detailLevel:   l.includes('nircam')  ? 1.0 :
                   l.includes('nirspec') ? 0.8 :
                   l.includes('niriss')  ? 0.7 :
                   l.includes('miri')    ? 0.6 : 0.3,
...
export class GenerativeBackground {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.ctxA = this.bufA.getContext('2d')! as OffscreenCanvasRenderingContext2D
    this.ctxB = this.bufB.getContext('2d')! as OffscreenCanvasRenderingContext2D
  ...
  setObservation(obs: ObservationData): void {
    ...
    const seed    = Math.floor(obs.ra * 1000 + (obs.dec + 90) * 100) + hashString(obs.targetName)
  ...
  private buildGalaxyCluster(
    ...
    const count = style.wideField ? 20 : style.minimal ? 5 : Math.floor(8 + rng() * 12)
    ...
    for (let i = 0; i < count; i++) {
      ...
      objects.push({
        ...
        rx: 0.025 + rng() * 0.07, ry: 0,
        angle: rng() * Math.PI,
        hue: h + (rng() - 0.5) * 30, sat: s, lit: l,
        alpha: 0.15 + rng() * 0.45,
        phase: rng() * Math.PI * 2,
        kind: 'galaxy',
      })
    ...
    for (let i = 0; i < arcCount; i++) {
      ...
      objects.push({
        x: massCx, y: massCy,
        rx: 0.10 + rng() * 0.18, ry: 0.03 + rng() * 0.07,
        angle: rng() * Math.PI,
        hue: h, sat: s, lit: l + 10,
        alpha: 0.08 + rng() * 0.18,
        phase: rng() * Math.PI * 2,
        kind: 'arc',
      })
  ...
  private buildNebula(
    ...
    for (let i = 0; i < cloudCount; i++) {
      ...
      objects.push({
        ...
        rx: 0.30 + rng() * 0.50, ry: 0,
        angle: 0,
        hue: h + (rng() - 0.5) * 20, sat: s, lit: l,
        alpha: 0.12 + rng() * 0.18,
        phase: rng() * Math.PI * 2,
        kind: 'cloud',
      })
    ...
    const filamentCount = style.detailLevel > 0.7
      ? Math.floor(6 + rng() * 8)
      : Math.floor(3 + rng() * 5)
    for (let i = 0; i < filamentCount; i++) {
      ...
      objects.push({
        ...
        rx: rng(), ry: rng(),
        angle: rng(),
        hue: h + (rng() - 0.5) * 40, sat: s * 0.8, lit: l + 20,
        alpha: 0.03 + rng() * 0.07,
        phase: rng() * Math.PI * 2,
        kind: 'filament',
      })
    ...
    objects.push({
      x: 0.35 + rng() * 0.30, y: 0.35 + rng() * 0.30,
      rx: 0.04 + rng() * 0.06, ry: 0,
      angle: 0,
      hue: gh, sat: gs, lit: gl + 15,
      alpha: 0.6 + rng() * 0.3,
      phase: rng() * Math.PI * 2,
      kind: 'core',
    })
  ...
  private buildStar(
    ...
    // Central star
    objects.push({
      x: 0.40 + rng() * 0.20, y: 0.40 + rng() * 0.20,
      rx: 0.025, ry: 0,
      angle: rng() * Math.PI / 6,
      hue: gh, sat: gs, lit: gl,
      alpha: 1.0,
      phase: rng() * Math.PI * 2,
      kind: 'star',
    })
    ...
    for (let i = 0; i < fieldCount; i++) {
      objects.push({
        x: rng(), y: rng(),
        rx: 0.001 + rng() * 0.003, ry: 0,
        angle: 0,
        hue: 200 + rng() * 60, sat: 30 + rng() * 40, lit: 70 + rng() * 25,
        alpha: 0.05 + rng() * 0.25,
        phase: rng() * Math.PI * 2,
        kind: 'fieldstar',
      })
  ...
  private buildDeepField(
    ...
    const objects: SceneObject[] = []
    const count = style.wideField ? 60 : style.minimal ? 20 : Math.floor(30 + rng() * 30)

    for (let i = 0; i < count; i++) {
      const [h, s, l] = rng() > 0.5 ? palette.primary : palette.secondary
      objects.push({
        x: rng(), y: rng(),
        rx: 0.005 + rng() * 0.020, ry: 0,
        angle: rng() * Math.PI,
        hue: h + (rng() - 0.5) * 40, sat: s, lit: l,
        alpha: 0.04 + rng() * 0.18,
        phase: rng() * Math.PI * 2,
        kind: 'galaxy',
      })
    ...
    for (let i = 0; i < brightCount; i++) {
      ...
      objects.push({
        x: 0.1 + rng() * 0.8, y: 0.1 + rng() * 0.8,
        rx: 0.015 + rng() * 0.04, ry: 0,
        angle: rng() * Math.PI,
        hue: h + (rng() - 0.5) * 15, sat: s, lit: l,
        alpha: 0.25 + rng() * 0.35,
        phase: rng() * Math.PI * 2,
        kind: 'galaxy',
      })
  ...
  private renderSceneToContext(
    ...
    for (const obj of scene.objects) {
      ...
      if (scene.style.thermalBlooms && (obj.kind === 'galaxy' || obj.kind === 'core')) {
        this.drawThermalBloom(octx, px, py, rx * 4.0, obj.hue, alpha * 0.35)
      }

      switch (obj.kind) {
        case 'galaxy':
          this.drawGalaxy(octx, px, py, rx, obj.angle, obj.hue, obj.sat, obj.lit, alpha, scene.style)
          break
        case 'arc':
          this.drawLensingArc(octx, px, py, obj.rx * minDim * this.sceneScale, obj.ry * minDim * this.sceneScale, obj.angle, obj.hue, obj.sat, obj.lit, alpha)
          break
        case 'cloud':
          this.drawNebulaCloud(octx, px, py, rx, obj.hue, obj.sat, obj.lit, alpha)
          break
        case 'filament':
          this.drawFilament(octx, w, h, obj, tSec)
          break
        case 'core':
          this.drawNebulaCore(octx, px, py, rx, obj.hue, obj.sat, obj.lit, alpha, glowPulse)
          break
        case 'star':
          this.drawCentralStar(octx, px, py, rx, obj.angle, obj.hue, obj.sat, obj.lit, minDim, scene.style, tSec, obj.phase)
          break
        case 'fieldstar':
          this.drawFieldStar(octx, px, py, rx, obj.hue, obj.sat, obj.lit, alpha)
          break
      }

      // Instrument over-layer: drawn after the object but sized and colored to match scene
      if (scene.style.spectraStreaks && (obj.kind === 'galaxy' || obj.kind === 'cloud')) {
        this.drawSpectraStreak(octx, px, py, rx, obj.angle, obj.hue, alpha, minDim)
      }
    }

    // Scene-wide diffuse haze
    const [ph, ps, pl] = scene.palette.primary
    const hazeGrad = octx.createRadialGradient(w * 0.5, h * 0.5, 0, w * 0.5, h * 0.5, minDim * 0.65)
  ...
  private drawGalaxy(
    ...
    if (style.detailLevel >= 0.9) {
      ...
      coreGrad.addColorStop(0, hsl(hue + 15, sat, Math.min(98, lit + 35), alpha * 0.8))
  ...
  private drawLensingArc(
    ...
    octx.rotate(angle)
    octx.beginPath()
    octx.ellipse(0, 0, Math.max(1, rx), Math.max(1, ry), 0, Math.PI * 0.25, Math.PI * 1.15)
  ...
  private drawNebulaCore(
    ...
    grad.addColorStop(0.2, hsl(hue, sat, Math.min(95, lit + 10), alpha * glowPulse))
  ...
  private drawCentralStar(
    ...
    if (bloomR > 0) {
      ...
      bloomGrad.addColorStop(0.1, hsl(hue, sat, Math.min(95, lit + 5), 0.9 * glowPulse))
    ...
    for (let i = 0; i < armCount; i++) {
      ...
      const grad = octx.createLinearGradient(0, 0, ax * spikeLen, ay * spikeLen)
