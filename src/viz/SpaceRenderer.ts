...
const RING_RADII = [0.18, 0.3, 0.44, 0.6];
...
export class SpaceRenderer {
  ...
  private render(): void {
    ...
    const parallaxX = (this.normalizedRA - 0.5) * 0.3 * parallaxScale;
    const parallaxY = (this.normalizedDec - 0.5) * 0.2 * parallaxScale;
    ...
    const hueShift =
      t * 0.03 + this.normalizedRA * 0.3 + this.normalizedDec * 0.2;
    ...
    if (this.aimX !== null && this.aimY !== null) {
      ...
      // Cross arms with gap
      ctx.beginPath();
      ctx.moveTo(rx - arm - gap, ry);
      ctx.lineTo(rx - gap, ry);
      ctx.moveTo(rx + gap, ry);
      ctx.lineTo(rx + arm + gap, ry);
      ctx.moveTo(rx, ry - arm - gap);
      ctx.lineTo(rx, ry - gap);
      ctx.moveTo(rx, ry + gap);
      ctx.lineTo(rx, ry + arm + gap);
      ctx.stroke();
