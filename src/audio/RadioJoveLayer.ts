...
//   galaxy / galaxy cluster → m74, m87, chorus
//   nebula / supernova      → jellyfish, tycho, chorus
//   star                    → tycho, chorus
//   default / unknown       → chorus, m74, jellyfish
//
...
function tracksForObservation(obs: ObservationData): TrackKey[] {
  ...
  if (type.includes('galaxy') || type.includes('cluster') || type.includes('quasar')) {
    return ['m74', 'm87', 'chorus']
  }
  if (type.includes('nebula') || type.includes('supernova') || type.includes('remnant')) {
    return ['jellyfish', 'tycho', 'chorus']
  }
  if (type.includes('star') || type.includes('stellar')) {
    return ['tycho', 'chorus']
  }
  return ['chorus', 'm74', 'jellyfish']
}
...
function buildGrainCloud(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const fbGain    = 0.25  + Math.random() * 0.30
...
export class RadioJoveLayer {
  ...
  private _activeTracks: TrackKey[] = ['chorus', 'm74', 'jellyfish']
  ...
  setSpace(value: number): void {
    ...
    this.dryGain.gain.setTargetAtTime(Math.max(0.02, 0.25 - value * 0.22), now, 0.4)
  ...
  setColour(value: number): void {
    ...
    this.filter.frequency.setTargetAtTime(120 * Math.pow(4000 / 120, value), now, 0.3)
  ...
  stop(): void {
    ...
    if (this.masterGain) {
      this.masterGain.gain.setTargetAtTime(0, this.masterGain.context.currentTime, 0.3)
    }
    try { this.lfo?.stop() } catch { /* already stopped */ }
    this.grainLines.forEach(g => { try { g.modOsc.stop() } catch { /* already stopped */ } })
  }
